...
export const calculateOverallChange = (player: Player): number => {
  ...
  if (age < peakStart) {
    if (age < 19) {
      newOverall = Math.min(potential, overall + Math.floor(0.65 * RANDINT(3, 5) * (19 - age)))
    } else {
      const COEFF = age < 23 ? 0.8 : 0.7;
      newOverall = Math.min(potential, overall + Math.floor(COEFF * RANDINT(4, 7 + (age - 18)) / (age - 18)))
    }
  }
